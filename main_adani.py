...
def option_hedge(client):
    ...
    df_NF = client.historical_data('N', 'C', 1594, '1m', str(from_),str(to_))
    df_BNF = client.historical_data('N', 'C', 25, '1m', str(from_),str(to_))
    ...
    for pos in client.positions():
        # print(pos)
        if 'ADANIENT' in pos['ScripName'] and 'CE'in pos['ScripName'] and pos['NetQty']>0:
            open_flag='BNF'
            break
        elif 'INFY' in pos['ScripName'] and 'CE'in pos['ScripName'] and pos['NetQty']>0:
            open_flag='NF'
            break
        else:
